...
Sketch.prototype.run = function(options) {
    if (this.frame)
        return;
    var {title, exit, resizable, resizeSuspend} = options || {};
    var instance = this;
...
Sketch.prototype.size = function() {
    // @@DOC initial resize (incl. renderer setting) should happen in setup()
    if (!this.applet.defaultSize && arguments.length == 2) {
        ...
        if (arguments[2] == constants.OPENGL && this.resizeSuspend !== false)
            this.resizeSuspend = true;
